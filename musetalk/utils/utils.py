...
def rand_log_normal(shape, loc=0.0, scale=1.0, device="cpu", dtype=torch.float32, generator=None):
    """Draws samples from an lognormal distribution."""
    rnd_normal = torch.randn(shape, device=device, dtype=dtype, generator=generator)  # N(0, I)
...
def get_mouth_region(frames, image_pred, pixel_values_face_mask):
    ...
    for b in range(frames.shape[0]):
        ...
        frames_resized = F.interpolate(
            frames_cropped.unsqueeze(0), size=(256, 256), mode="bilinear", align_corners=False
        )
        image_pred_resized = F.interpolate(
            image_pred_cropped.unsqueeze(0), size=(256, 256), mode="bilinear", align_corners=False
        )
    ...
    with torch.no_grad():
        ...
        audio_feats = torch.cat(
            [
                ...
                audio_feats,
                torch.zeros_like(audio_feats[:, : 2 * cfg.data.audio_padding_length_right]),
            ],
            1,
        )
        audio_prompts = []
